...
const badgeImgs: Element[] = Array.from(
  document.getElementsByClassName("badge-img")
);
...
export function updateBadgeAppearance(
  elements: Element[],
  profile: Badges
): void {
  ...
  for (let best of bestBadges) {
    if (Number(best[2]) > 0) {
      for (let element of elements) {
        ...
        let operator: string | null = element.getAttribute(
          "data-badge-operator"
        );
