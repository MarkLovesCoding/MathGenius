...
window.onload = function () {
  ...
  const operatorTestMenuBackwardLower = <HTMLElement>(
    document.getElementById("operator-test-menu-backward-lower")
  );
  const difficultyMenuForward = <HTMLElement>(
    document.getElementById("difficulty-menu-forward")
  );
  const difficultyMenuBackwardLower = <HTMLElement>(
    document.getElementById("difficulty-menu-backward-lower")
  );

  const activitiesChoices = <NodeListOf<Element>>(
    document.querySelectorAll(".activity-choice")
  );
  const realChoices = <NodeListOf<Element>>(
    document.querySelectorAll(".real-choice")
  );
  const operatorContainer = <HTMLElement>(
    document.getElementById("operator-menu-container")
  );
  const activityContainer = <HTMLElement>(
    document.getElementById("real-menu-container")
  );
  const difficultyContainer = <HTMLElement>(
    document.getElementById("difficulty-menu-container")
  );
  const operatorChoices = <NodeListOf<Element>>(
    document.querySelectorAll(".operator-choice")
  );
  ...
  function addEventsForTypesOrActivities(
    activitiesChoices: NodeListOf<Element>
  ) {
    for (let activity of activitiesChoices) {
      activity.addEventListener("click", () => {
        activitiesChoices.forEach((activity) =>
          activity.classList.remove("activity-selected")
        );
  ...
  function addEventsForOperators(operatorsChoices: NodeListOf<Element>) {
    for (let operator of operatorsChoices) {
      operator.addEventListener("click", () => {
        operatorsChoices.forEach((operator) =>
