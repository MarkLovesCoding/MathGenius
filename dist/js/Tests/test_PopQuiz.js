    ...
    if (quizStats.numAnswered >= quizStats.numQuestions) {
        ...
        if (quizStats.numCorrect == 10) {
            ...
            if (activeDifficulty && activeOperator)
                ...
                throw new Error("Session storage operator and difficulty not retrieved");
            }
        }
        else {
            quizShowScore();
...
function quizAnswerHandler(e) {
    ...
    let realAns = utilMethods.calculation(parseInt(quizNumOne.innerHTML), parseInt(quizNumTwo.innerHTML), quizOpOne.innerHTML); // Calculate the correct answer using the quizNumOne, quizNumTwo, and quizOpOne elements
