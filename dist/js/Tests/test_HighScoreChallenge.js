...
const gameAnswerSubmit = (document.getElementById("game-answer-form"));
const gameCorrectness = (document.getElementById("game-correctness"));
const gameActual = document.getElementById("game-actual");
const gameActualContainer = (document.getElementById("game-actual-container"));
const gameCurrScore = (document.getElementById("game-current-score"));
const gameHighScore = document.getElementById("game-current-high");
const gameLevelNumber = (document.getElementById("game-level-number"));
const gameTracker = document.getElementById("game-tracker");
const gameTrackerContainer = (document.getElementById("game-tracker-container"));
////////////////////////////////////////////////////////////
//GAME
//
async function gameAnswerCheck(bool) {
    ...
    if (bool) {
        ...
        if (operator)
            ...
            throw new Error("Error retrieving session storage Data");
    }
...
async function checkBadgeStatus(badges, operator, difficulty) {
    ...
    if (badges[operator]["game"][difficulty] === false)
        return true;
    else
        return false;
}
async function updateLevel() {
    ...
    const currScore = parseInt(gameCurrScore.textContent);
    if (currScore % 1 == 0) {
    ...
    if (currScore % 10 == 0) {
        ...
        await utilMethods.updateLevelVisuals(nextDifficulty.toString());
...
async function updateProgress(questionNumber) {
    ...
    let fullWidthNum = parseFloat(fullWidth.slice(0, -2));
    let borderWidthNum = parseFloat(borderWidth.slice(0, -2));
    ...
    let progressWidth = window.getComputedStyle(gameTracker).width;
    let progressWidthNum = parseFloat(progressWidth.slice(0, -2));
...
function gameCheckAnswerHandler(e) {
    let realAns = utilMethods.calculation(parseInt(gameNumOne.innerHTML), parseInt(gameNumTwo.innerHTML), gameOpOne.innerHTML); // Calculate the correct answer using the gameNumOne, gameNumTwo, and gameOpOne elements
...
async function resetGameSettings() {
    ...
    if (operator) {
        ...
        throw new Error("Error: Session Storage couldn't be retrieved");
    await utilMethods.delay(250);
    utilMethods.enableInput(gameAnswerInput);
}
