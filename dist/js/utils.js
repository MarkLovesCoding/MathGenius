 ...
 */
export function setHighLowVals(difficulty, operator) {
    let highVal;
    let lowVal;
    ...
    if (operator === "x") {
        switch (difficulty) {
            ...
            case "5":
                ...
                break;
            default:
                const exhaustiveCheck = difficulty;
                return exhaustiveCheck;
        }
    }
    else if (operator === "+") {
...
export function updateDifficultyRange() {
    // let  highVal, lowVal;
...
export function closeAlert(element) {
    element.style.display = "none";
}
export async function retrieveAvatar(element) {
    try {
