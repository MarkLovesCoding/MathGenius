describe("pop quiz spec", () => {
  ...
  it("should check full pop quiz test flow and content", () => {
    cy.get("div#new-quiz").should("exist").click();
    ...
    cy.get("div#selected-activity-menu-diff").should("have.text", "Pop Quiz");
    ...
    cy.get("input#range-input").invoke("val", 1).trigger("input");
    cy.get("#level-text").should("have.text", "Easy");
    cy.get("input#range-input").invoke("val", 2).trigger("input");
    cy.get("#level-text").should("have.text", "Novice");
    cy.wait(200);
    ...
    cy.wait(200);
    cy.get("div#quiz-container").should("exist");
    cy.window().then((win) => {
      ...
      expect(activity).to.equal("quiz");
      expect(activeOperators).to.equal("x");
      // Unit testing fills rest of checks.
    });
    cy.get("h2#activity-title-quiz").should("contain", "Pop Quiz");
