...
router.post("/auth/guest", passport.authenticate("guest"), (req, res) => {
  res.redirect("/");
});
...
router.post("/forgot-username", async (req, res) => {
  try {
    ...
    req.flash("flashAlert", {
      type: "success",
      message: "Username sent to your email. Please check your inbox.",
    });
    return res.redirect("/login");
  } catch (err) {
    console.log(err);
    req.flash("flashAlert", {
      type: "error",
      message: "Error retrieving your username. Please try again.",
    });
    return res.redirect("/forgot-password");
  }
});
...
router.post("/forgot-password", limiter, async (req, res) => {
  ...
  try {
    ...
    req.flash("flashAlert", {
      type: "error",
      message: "Error occurred resetting your password.",
    });
    return res.redirect("/forgot-password");
  }
});
  ...
  try {
    ...
    req.logIn(user, (err) => {
      if (err) {
        req.flash("flashAlert", {
          type: "error",
          message: "Error logging in with new password.",
        });
        return res.redirect("/login");
      } else {
        req.flash("flashAlert", {
